// scripts/generate-shop-images-commonjs.ts
...
// Get project root directory (CommonJS compatible)
const __dirname = path.dirname(__filename || require.main?.filename || process.cwd());
...
async function main() {
  ...
  // Check for API keys (priority order: DALL-E > Grok > Stable Diffusion > Placeholder)
  const openaiKey = process.env.OPENAI_API_KEY;
  const grokKey = process.env.GROK_API_KEY || process.env.XAI_API_KEY;
  const huggingfaceKey = process.env.HUGGINGFACE_API_KEY;
  
  const useDALLE = !!openaiKey;
  const useGrok = !!grokKey && !useDALLE;
  const useStableDiffusion = !!huggingfaceKey && !useDALLE && !useGrok;
  const usePlaceholder = !useDALLE && !useGrok && !useStableDiffusion;

  console.log('📋 Configuration:');
  console.log(`   - OpenAI DALL-E: ${useDALLE ? '✅ Available' : '❌ Not configured'}`);
  console.log(`   - Grok (xAI): ${useGrok ? '✅ Available' : '❌ Not configured'}`);
  ...
  if (usePlaceholder) {
    ...
    console.log('💡 To generate real images, set one of these in .env:');
    console.log('   - OPENAI_API_KEY (DALL-E)');
    console.log('   - GROK_API_KEY or XAI_API_KEY (Grok)');
    console.log('   - HUGGINGFACE_API_KEY (Stable Diffusion)\n');
  }
  ...
  try {
    ...
    for (let i = 0; i < shops.length; i++) {
      ...
      // Grok rate limit: 5 requests/second (200ms between requests minimum)
      // DALL-E: ~2s delay, Grok: ~250ms, Stable Diffusion: ~1s
      if ((useDALLE || useGrok || useStableDiffusion) && i < shops.length - 1) {
        let delay = 1000; // Default 1s
        if (useDALLE) {
          delay = 2000; // DALL-E is slower
        } else if (useGrok) {
          delay = 250; // Grok rate limit is 5 req/s = 200ms minimum, use 250ms for safety
        } else if (useStableDiffusion) {
          delay = 1000; // Stable Diffusion
        }
