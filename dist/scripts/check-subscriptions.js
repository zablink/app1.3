...
async function checkSubscriptions() {
    try {
        ...
        const shopsWithSubs = await prisma.$queryRaw `
      SELECT 
        s.id, s.name, sp.tier,
    ...
    `;
        ...
        const apiTestQuery = await prisma.$queryRaw `
    ...
    `;
        ...
        const grouped = apiTestQuery.reduce((acc, shop) => {
        ...
        Object.keys(grouped).sort().forEach(tier => {
            ...
            grouped[tier].slice(0, 3).forEach((shop) => {
        ...
        const apiTestQuery3 = await prisma.$queryRaw `
      ...
      LIMIT 50
    `;
        const grouped3 = apiTestQuery3.reduce((acc, shop) => {
            const tier = shop.subscriptionTier || 'FREE';
            if (!acc[tier])
                acc[tier] = [];
            acc[tier].push(shop);
        ...
        Object.keys(grouped3).sort().forEach(tier => {
            ...
            grouped3[tier].slice(0, 3).forEach((shop) => {
                console.log(`  - ${shop.name} (${shop.id})`);
            });
