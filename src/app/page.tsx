      ...
      <main className="container mx-auto px-4 py-6">
        ...
        {!isLoadingShops && shops.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {shops.map((shop) => (
              <div key={shop.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer" onClick={() => router.push(`/shop/${shop.id}`)}>
                {/* Image (logo or image) */}
                <div className="w-full h-48 bg-gray-200 rounded-t-lg overflow-hidden">
                  <img src={shop.logo || shop.image || '/placeholder.png'} alt={shop.name} className="w-full h-full object-cover" />
                </div>
                <div className="p-4">
